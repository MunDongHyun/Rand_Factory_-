...
- 현재 `article_thumbnail_filename` 채워진 16건 모두 디스크 파일 존재 + `article_thumbnail_url` 정상 생성 확인
...
- 백엔드 응답 검증: `_to_response(article)` 기준 `article_thumbnail_url` 생성 row 16건 확인

### 보류
- 남은 미매핑 PNG 14건: 같은 글의 reframe인지, DB INSERT 누락인지 수동 검토 필요
  - `고객 신뢰 구축...`, `고객 신뢰를 위한 투명성 전략...`은 `2080` 후보가 될 수 있으나 둘 중 어느 파일이 맞는지 확인 필요
  - `우크라이나 전쟁 이후...`, `해상 초크 포인트...`는 `2096` 후보가 될 수 있으나 1:1 정책상 선택 필요
  - 조직/협업/리더십 계열 PNG는 `2073`, `2068`, `2097`, `2093` 후보가 섞여 있어 원문 확인 필요
- 나머지 articles 22건은 아직 `article_thumbnail_filename` NULL
