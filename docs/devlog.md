...
### 변경
- `client/src/styles/Curriculum.css`: `.loadingModalOverlay` z-index 99998 → 100000 분리
  - 기존엔 다른 오버레이(chat/confirm/download)와 같은 99998로 묶여 있어 로딩 모달이 뒤로 깔리는 케이스 발생 가능
  - 컨테이너는 99999 그대로 유지, 오버레이만 위로 올려 다른 모달 위에 띄움
- `docs/integration_test_2026_05_28.md`: 5/29 직접 시연 검증한 항목 체크
  - 사전 준비 / 1-A·B·D·E·F / 2-A·C(일부)·D·E(일부)·F·G 통과 표시
  - 결과 정리 🔴 이슈 4건 (재제출 첨부 자동 승계, 커리큘럼 진행률+발급 자격, 매니저 승급 회사명, 학습자 알림 딥링크) → 모두 직전 사이클 픽스 완료
  - 진행자/일자(2026.05.29 정찬희) 기입
  - 남은 미체크: 1-C 커리큘럼 생성/양식 배포, 1-G/H 수료증 발급, 2-B 양식 잠금, 2-E 권한 차단(3건), R2 prefix 누적 확인, 시연 환경 안정성

---

## 2026-05-29 - Codex (수료증 양식 정리 + 마감일/피드백 운영 UX 보강)

### 변경
- 수료증 미리보기 문구 정리
  - 매니저 학습자관리의 `샘플 수료증` 버튼을 `수료증 양식`으로 변경
  - 실제 발급과 동일한 PDF 생성기를 사용하되 DB/R2 저장 없이 양식 확인용으로 다운로드
  - 미리보기 파일명/번호도 `sample` 대신 `preview` 기준으로 정리
- 커리큘럼 과제 마감일 판정 보정
  - 신규 저장 마감일은 선택 날짜의 `23:59:59`까지 제출 가능하도록 변경
  - 기존에 `00:00:01` 형태로 저장된 마감일도 서버 제출 판정에서 해당 날짜의 끝까지 허용하도록 보정
  - 학습자 제출 실패 시 인라인 에러와 함께 토스트도 표시
- 매니저 커리큘럼 관리 운영 UX 보강
  - 배포 완료 과제는 양식 내용은 잠그고 `마감일 변경`만 가능하게 처리
  - 피드백 저장 후 같은 학습자의 다음 `피드백 대기` 과제로 자동 이동

### 검증
- `server\venv\Scripts\python.exe -m py_compile app\routers\certificate.py app\routers\curriculum.py app\routers\task_submission.py` 통과
- 기존/신규 마감일 문자열이 서버에서 동일하게 해당일 `23:59:59 KST`로 보정되는지 직접 확인
- `client`에서 `npm run build` 통과
