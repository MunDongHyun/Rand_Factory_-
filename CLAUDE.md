...
(DB의 `chatbot_sessions`, `chatbot_messages` 테이블은 ORM/라우터에서 제거됨. 데이터 보존 차원에서 테이블 자체는 남겨두며, 드롭 여부는 별도 결정)
...
- `framework`
- `chat`
- `chatbot` — 2026-05-19 결정으로 백엔드 라우터/모델/스키마 제거 (2026-05-20)
...
| `m` | 매니저 (manager) | OJT 결제로 승급한 회원. 본인 회사 커리큘럼 생성/수정, 과제 피드백, 학습자 초대 |
...
- 일반 회원(`c`)은 커리큘럼/과제 라우터 차단 — 라우터별 권한 매트릭스 점검은 별도 사이클 (현재 라우터 코드는 `c` 역할을 모르므로 점검 필요)
