...
- `POST /api/users/signup/bulk`로 회사 + 매니저 1명(첫 번째) + 학습자 N명을 한 번에 등록 (MVP 정책, 인증 없음)
  - 첫 번째 직원이 매니저(`m`), 나머지는 모두 학습자(`j`)로 강제 부여
  - `user_company`는 요청의 `company` 값으로 전원 동일하게 부여
  - 같은 회사명으로 누구나 가입 가능 (보안 강화는 V2에서 검토)
- 기존 `POST /api/users/signup`은 호환성 위해 유지하되 UI에서 호출하지 않음 (단일 학습자 가입용 백업 경로)
